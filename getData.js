...
async function getLendingData(userAddress,contractAddress){
    ...
    let LendedAmount=await LendingInstance.methods.getCdpTotalDeposited(userAddress).call();
    let borrowedAmount=await LendingInstance.methods.getCdpTotalDebt(userAddress).call();
    ...
    {
        console.log('Lended amount:',(LendedAmount/10**lendedDecimals).toFixed(2),lendedSymbol);
        console.log('Borrowed Amount:',(borrowedAmount/10**lendedDecimals).toFixed(2),borrowedSymbol)
    }    
}
...
async function getLPdata(userAddress){
    ...
    {
        console.log(symbol0,'+',symbol1,token0amount.toFixed(2),'+',token1amount.toFixed(2));
...
async function getCRVpoolData(userAddress){
    ...
    let rewards=await masterInstance.methods.calculateIncome(1,userAddress).call();
...
async function getdStakeData(userAddress,index){
    ...
    let rewards=await masterInstance.methods.calculateIncome(index,userAddress).call();
    let userInfo=await masterInstance.methods.userInfos(index,userAddress).call();
